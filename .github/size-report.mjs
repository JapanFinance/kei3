...
// lower quality). The total matches size-limit's former figure to the byte.
...
// Deliberately tight — ~2 kB over the current total — so any real increase
// fails CI. Raise it in the same change that legitimately adds the weight.
