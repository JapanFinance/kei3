...
- Very low incomes are now presented honestly. Income of exactly ¥0 previously showed every charge as ¥0 while ¥1 of income charged everything; ¥0 now computes the real fixed charges under the selected provider, consistently with any small income. When fixed social insurance charges exceed income, the take-home figure is shown as a warning (instead of green) with an explanation of which charges cause it and what relief exists — including the National Pension full exemption and, for employee providers at very low salaries, a note that enrollment as a short-time worker has statutory criteria ([短時間労働者に対する健康保険・厚生年金保険の適用の拡大](https://www.nenkin.go.jp/service/kounen/tekiyo/jigyosho/tanjikan.html)) and that National Health Insurance usually applies instead. Percentage displays no longer show "100%" or invalid values at ¥0 income, and the chart's Take-Home % line no longer produces an invalid point at ¥0.
