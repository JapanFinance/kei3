...
- Added a household type selector to the income distribution chart. The median line and the percentile estimate can now be measured against any of the six household types (世帯類型) the survey publishes: 全世帯 (all households), 高齢者世帯 (elderly households), 高齢者世帯以外の世帯 (non-elderly households), 65歳以上の者のいる世帯 (households with a member aged 65 or over), 児童のいる世帯 (households with children), and 母子世帯 (single-mother households). A ¥5,000,000 income ranks above ~54% of all households but above only ~39% of 高齢者世帯以外の世帯, since the all-households figure includes 高齢者世帯. Each type's definition and median are in the tooltip next to the selector, taken from the survey's [用語の説明](https://www.mhlw.go.jp/toukei/saikin/hw/k-tyosa/k-tyosa25/dl/07.pdf).
...
- The income distribution now resolves income in ¥500,000 steps up to ¥10,000,000, rather than ¥1,000,000 steps, making the percentile estimate more precise across the range where most households sit. The figures come from [所得票 第０２１表](https://www.e-stat.go.jp/stat-search/files?toukei=00450061&tstat=000001244376&cycle=7&tclass1=000001244380) of the 2025 survey's statistical tables, which reports every household type against the same brackets.
- An income above ¥20,000,000 is now described as being in the top ~1.6% rather than as higher than a precise-looking ~98.4%. The survey's top bracket is open-ended at 20,000,000円以上 and publishes no detail within it, so every income above that floor shares one figure.
- The quintile background bands always show the 全世帯 (overall-population) income quintiles as a fixed reference, even when the percentile is measured against another household type. The survey publishes 五分位値 for 全世帯 alone — reporting how each household type spreads across those same bands rather than giving each type its own boundaries — so the bands stay labeled as 全世帯 to keep them distinct from the selected comparison group.
