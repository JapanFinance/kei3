...
- The quintile background bands always show the all-households (全世帯) income quintiles as a fixed reference, even when the percentile is measured against another household type. The survey publishes these quintile boundaries for all households alone — reporting how each household type spreads across those same bands rather than giving each type its own — so the bands stay labeled as all-households to keep them distinct from the selected comparison group.
