...
interface IncomeModeChangedAction {
  ...
  /**
   * Id for the fresh stream created when entering advanced mode with no restorable
   * streams. Generated by the caller so the reducer stays free of
   * `Date.now()`/`Math.random()` nondeterminism.
   */
  newStreamId: string;
}
    ...
    if (totalAnnualIncomeFromStreams(candidate) === state.annualIncome) {
      ...
      newState.incomeStreams = [
        hasEmploymentIncome(state)
          ? {
              id: action.newStreamId,
              type: 'salary',
              frequency: 'annual',
              amount: state.annualIncome,
            }
          : { id: action.newStreamId, type: 'miscellaneous', amount: state.annualIncome },
      ];
    }
  }
