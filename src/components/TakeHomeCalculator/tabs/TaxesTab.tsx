    ...
    <Box>
      ...
      <Box sx={{ mb: 1 }}>
        ...
        {results.residenceTax.nonTaxableStatus ? (
          // The breakdown rows would show a positive taxable income beside all-zero tax rows,
          // so replace them with the reason the exemption applies.
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            {`Non-taxable: ${NON_TAXABLE_STATUS_LABELS[results.residenceTax.nonTaxableStatus]} with 合計所得金額 of ${formatJPY(NON_TAXABLE_STATUS_INCOME_LIMIT)} or less are exempt from residence tax.`}
          </Typography>
        ) : (
          <>
