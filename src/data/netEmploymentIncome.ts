 ...
 */
export const getEmploymentIncomeDeductionPeriod = (
  year: number,
): EmploymentIncomeDeductionPeriod => {
...
export const calculateIncomeAdjustmentDeductionAmount = (grossEmploymentIncome: number): number => {
  ...
  const cappedEmploymentIncome = Math.min(
    grossEmploymentIncome,
    INCOME_ADJUSTMENT_EMPLOYMENT_INCOME_CAP,
  );
