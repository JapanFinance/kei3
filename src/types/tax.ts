...
export interface HomeLoanTaxCreditResult {
  ...
  /** Portion that spilled over and was applied against the residence-tax 所得割. */
   ...
   * by at most `applied` = min(flatCap 定額限度, incomeRateCap 定率限度, residenceIncomeBasedCap).
   */
  residenceTaxSpilloverCap?: {
    /** The binding cap actually used: min(flatCap, incomeRateCap, residenceIncomeBasedCap). */
    applied: number;
    ...
    /**
     * The residence-tax income-based portion (所得割) levied before the credit — the pool the
     * spillover is deducted from. Zero when residence tax is 非課税 or only 均等割 is levied,
     * which makes the whole spillover unusable however much statutory headroom remains.
     */
    residenceIncomeBasedCap: number;
  };
