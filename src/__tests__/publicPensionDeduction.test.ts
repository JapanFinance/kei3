...
const YEAR = 2026;
const UNDER_65: AgeRange = { minAgeInclusive: 0, maxAgeExclusive: 65 };
const AGE_65_PLUS: AgeRange = {
  minAgeInclusive: 65,
  maxAgeExclusive: Number.POSITIVE_INFINITY,
};
...
describe('calculateNetPublicPensionIncome', () => {
  describe('under 65', () => {
    const recipientAgeRange = UNDER_65;

    it('returns zero at or below the ¥600,000 minimum deduction', () => {
      // The statutory table's first row: gross ≤ 600,000 → 雑所得 0
      expect(calculateNetPublicPensionIncome(600_000, recipientAgeRange, 0, YEAR)).toBe(0);
      expect(calculateNetPublicPensionIncome(600_001, recipientAgeRange, 0, YEAR)).toBe(1);
    });
    ...
    it('applies the minimum deduction up to the first tier boundary', () => {
      ...
      expect(calculateNetPublicPensionIncome(1_300_000, recipientAgeRange, 0, YEAR)).toBe(700_000);
    });

    it('applies the 75% tier', () => {
      // 1,400,000 × 75% − 275,000 = 775,000
      expect(calculateNetPublicPensionIncome(1_400_000, recipientAgeRange, 0, YEAR)).toBe(775_000);
    });

    it('is continuous at the 75%/85% boundary', () => {
      // 4,100,000 × 75% − 275,000 = 2,800,000 = 4,100,000 × 85% − 685,000
      expect(calculateNetPublicPensionIncome(4_100_000, recipientAgeRange, 0, YEAR)).toBe(
        2_800_000,
      );
    });
    ...
    it('is continuous at the 85%/95% boundary', () => {
      ...
      expect(calculateNetPublicPensionIncome(7_700_000, recipientAgeRange, 0, YEAR)).toBe(
        5_860_000,
      );
    });
    ...
    it('drops any fraction of a yen from the 雑所得', () => {
      ...
      expect(calculateNetPublicPensionIncome(2_500_001, recipientAgeRange, 0, YEAR)).toBe(
        1_600_000,
      );
      // 2,500,003 × 75% − 275,000 = 1,600,002.25 → 1,600,002
      expect(calculateNetPublicPensionIncome(2_500_003, recipientAgeRange, 0, YEAR)).toBe(
        1_600_002,
      );
    });
    ...
    it('caps the deduction at ¥1,955,000 above ¥10,000,000', () => {
      ...
      expect(calculateNetPublicPensionIncome(10_000_000, recipientAgeRange, 0, YEAR)).toBe(
        8_045_000,
      );
      // 12,000,000 − 1,955,000 = 10,045,000
      expect(calculateNetPublicPensionIncome(12_000_000, recipientAgeRange, 0, YEAR)).toBe(
        10_045_000,
      );
    });
  });
  ...
  describe('65 or older', () => {
    const recipientAgeRange = AGE_65_PLUS;

    it('returns zero at or below the ¥1,100,000 minimum deduction', () => {
      // The statutory table's first row: gross ≤ 1,100,000 → 雑所得 0
      expect(calculateNetPublicPensionIncome(1_100_000, recipientAgeRange, 0, YEAR)).toBe(0);
      expect(calculateNetPublicPensionIncome(1_100_001, recipientAgeRange, 0, YEAR)).toBe(1);
    });

    it('applies the higher ¥1,100,000 minimum deduction', () => {
      // 1,500,000 − 1,100,000 = 400,000
      expect(calculateNetPublicPensionIncome(1_500_000, recipientAgeRange, 0, YEAR)).toBe(400_000);
    });

    it('is continuous at the minimum/75% boundary', () => {
      // 3,300,000 − 1,100,000 = 2,200,000 = 3,300,000 × 75% − 275,000
      expect(calculateNetPublicPensionIncome(3_300_000, recipientAgeRange, 0, YEAR)).toBe(
        2_200_000,
      );
    });
    ...
    it('matches the under-65 table above the minimum tiers', () => {
      ...
      expect(calculateNetPublicPensionIncome(4_000_000, recipientAgeRange, 0, YEAR)).toBe(
        2_725_000,
      );
      expect(calculateNetPublicPensionIncome(4_000_000, UNDER_65, 0, YEAR)).toBe(2_725_000);
    });
  });
  ...
  describe('non-pension income bands', () => {
    it('keeps the full deduction at exactly ¥10,000,000 of other income', () => {
      ...
      const atThreshold = calculateNetPublicPensionIncome(4_000_000, UNDER_65, 10_000_000, YEAR);
      expect(atThreshold).toBe(calculateNetPublicPensionIncome(4_000_000, UNDER_65, 0, YEAR));
    ...
    it('reduces the deduction by ¥100,000 above ¥10,000,000 of other income', () => {
      ...
      expect(calculateNetPublicPensionIncome(4_000_000, UNDER_65, 10_000_001, YEAR)).toBe(
        2_825_000,
      );
    });
    ...
    it('reduces the deduction by ¥200,000 above ¥20,000,000 of other income', () => {
      ...
      expect(calculateNetPublicPensionIncome(4_000_000, UNDER_65, 20_000_001, YEAR)).toBe(
        2_925_000,
      );
    });
    ...
    it('applies the reduced band floor to small pensions', () => {
      ...
      expect(calculateNetPublicPensionIncome(550_000, UNDER_65, 10_000_001, YEAR)).toBe(50_000);
    });
    ...
    it('shifts the zero threshold down along with the deduction', () => {
      ...
      expect(calculateNetPublicPensionIncome(400_000, UNDER_65, 20_000_001, YEAR)).toBe(0);
      expect(calculateNetPublicPensionIncome(400_001, UNDER_65, 20_000_001, YEAR)).toBe(1);
    });
  });
});
