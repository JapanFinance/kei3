...
describe('getHomeLoanTaxCreditCohort', () => {
    ...
    it('returns the 2009-2013 band', () => {
        expect(getHomeLoanTaxCreditCohort(2009)?.moveInYearFrom).toBe(2009);
        expect(getHomeLoanTaxCreditCohort(2013)?.moveInYearFrom).toBe(2009);
        const band = getHomeLoanTaxCreditCohort(2010)!;
        expect(band.incomeLimit).toBe(30_000_000);
        expect(band.spillover).toEqual({ flatCap: 97_500, taxableIncomeRate: 0.05 });
    });
    ...
    it('returns undefined for unsupported (pre-2009) move-ins', () => {
        ...
        expect(getHomeLoanTaxCreditCohort(1999)).toBeUndefined();
    });
});
